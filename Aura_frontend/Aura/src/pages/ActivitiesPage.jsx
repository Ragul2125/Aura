...
export default function ActivitiesPage() {
  ...
  const fetchQuestions = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        "https://56d2bc5119c8.ngrok-free.app/analyze_memory",
        {
      ...
      if (response.data && Array.isArray(response.data)) {
        ...
        console.warn(
          "Unexpected API response format, using fallback questions",
        );
      ...
      setError(
        "Unable to load personalized questions. Using default questions.",
      );
      setQuestions(FALLBACK_QUESTIONS);
    } finally {
      setLoading(false);
  ...
  const submitAnswers = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://56d2bc5119c8.ngrok-free.app/submit_answers",
        {
          user_id: userId,
  ...
  const handleSelect = (questionId, option) => {
    ...
    if (activeQuestion < questions.length - 1) {
      setTimeout(() => {
        document
          .getElementById(`question-${activeQuestion + 1}`)
          ?.scrollIntoView({
            behavior: "smooth",
            block: "nearest",
            inline: "center",
      ...
      <div className="relative w-full">
        ...
        >
          {questions.map((q, index) => (
            ...
            >
              <div
                className={`h-full rounded-3xl p-6 shadow-sm border-2 ${q.color} transition-all duration-300 bg-white`}
              >
                <div className="mb-6">
        ...
        {questions.map((_, idx) => (
          <div
            ...
            className={`h-2 rounded-full transition-all duration-300 ${
              idx === activeQuestion ? "w-8 bg-gray-800" : "w-2 bg-gray-300"
            }`}
          />
        ))}
      </div>
